...
% ===========================  START of Task 1: Calculate AoD ==============
    % Hint:  Find the actual angle θ1 and θ2 for user 1 and user 2
    AoD = atan2d(rx_location(:,2) - tx_location(2), rx_location(:,1) - tx_location(1)); % 計算發射角度
    % dprintf(sprintf('User 1 original AoD: %.2f degrees\n', AoD(1)));
    % dprintf(sprintf('User 2 original AoD: %.2f degrees\n', AoD(2)));
    printf(sprintf('Rx1 actual Angle: %.6f degree\n', AoD(1)));
    printf(sprintf('Rx2 actual Angle: %.6f degree\n', AoD(2)));
    AoD = mod(AoD, 180); % 限制在 0~180 度範圍內

    % Hint:  Find the index of the beam direction that is closest to the actual angle
    [~, idx1] = min(abs(tx_beam_direction - AoD(1)));
    [~, idx2] = min(abs(tx_beam_direction - AoD(2)));
    optimal_beam_direction = [tx_beam_direction(idx1), tx_beam_direction(idx2)];
    % printf(sprintf('User 1 AoD: %.2f degrees, Closest Beam: %.2f degrees\n', AoD(1), optimal_beam_direction(1)));
    % printf(sprintf('User 2 AoD: %.2f degrees, Closest Beam: %.2f degrees\n', AoD(2), optimal_beam_direction(2)));
...
% ===========================  START of Task 2: Calculate Rx power and SNR ==============
    ...
    psi = 2 * pi * d * cos(phi); % Phase shift
% ==========================================================================================

    % beam_resolutions = linspace(0, 180, resolution); % Beam resolution
    beam_resolutions = (0.5:(180/resolution):180); % Beam resolution
    [~, AoD1_resolutions_idx] = min(abs(beam_resolutions - AoD(1))); % AoD1 resolution
    [~, AoD2_resolutions_idx] = min(abs(beam_resolutions - AoD(2))); % AoD2 resolution
    optimal_beam_resolutions_idx = [AoD1_resolutions_idx, AoD2_resolutions_idx]; % Optimal beam resolution index
    optimal_beam_resolutions = [beam_resolutions(AoD1_resolutions_idx), beam_resolutions(AoD2_resolutions_idx)]; % Optimal beam resolution
    % rx_Sector_idxs = zeros(rx_node_number, 1);
    % interference_Sector_idxs = zeros(rx_node_number, 1);
    ...
    G_interference = zeros(rx_node_number, 1);

    SNR_dB = zeros(rx_node_number, 1);
    INR_dB = zeros(rx_node_number, 1);

    rx_Dists = zeros(rx_node_number, 1);
    rx_Beam_idxs = zeros(rx_node_number, 1);
    interference_Beam_idxs = zeros(rx_node_number, 1);
    
    for i = 1:rx_node_number
        dprintf(sprintf('Rx%d optimal beam resolution: %.6f degrees\n', i, optimal_beam_resolutions(i)));

        [weights, hpdw] = uniform(d, optimal_beam_direction(i), tx_antenna_number); % 計算權重
        Gain = abs(dtft(weights, -psi)).^2; % 計算 Gain
        Gains(i, :) = Gain; % 儲存 Gain for report use
        
        % rx_Sector_idxs(i) = round(AoD(i) * resolution / 180); % 計算 Rx 所在的 sector index
        % interference_Sector_idxs(i) = round(AoD(3 - i) * resolution / 180); % 計算 interference 所在的 sector index
        % dprintf(sprintf('Rx%d sector angle: %f degree\n', i, beam_resolutions(rx_Sector_idxs(i))));
        % if (beam_resolutions(rx_Sector_idxs(i)) == beam_resolutions(optimal_beam_resolutions_idx(i)) && ...
        %     beam_resolutions(interference_Sector_idxs(i)) == beam_resolutions(optimal_beam_resolutions_idx(3 - i)))
        %     printf(sprintf('=== Verification pass ===\n'));
        % else
        %     printf(sprintf('=== Verification fail ===\n'));
        % end
        
        rx_Dists(i) = norm(rx_location(i, :) - tx_location); % 計算 Tx 到 Rx 的距離
        [~, rx_Beam_idxs(i)] = max(Gain(:, optimal_beam_resolutions_idx(i))); % 計算 Rx 所在的 beam index
        [~, interference_Beam_idxs(i)] = max(Gain(:, optimal_beam_resolutions_idx(3 - i))); % 計算 interference 所在的 beam index

        G_tx(i) = Gain(rx_Beam_idxs(i), optimal_beam_resolutions_idx(i)); % 計算 Tx Gain
        G_interference(i) = Gain(interference_Beam_idxs(i), optimal_beam_resolutions_idx(3 - i)); % 計算 interference Gain
        % dprintf(sprintf('User %d Tx Gain: %.6f dB\n', i, G_tx(i)));

        SNR_dB(i) = P_tx_dBm + friis_equation(freq, G_tx(i), 1, rx_Dists(i)) - N0_dBm; % 計算 SNR
        INR_dB(i) = P_tx_dBm + friis_equation(freq, G_interference(i), 1, rx_Dists(i)) - N0_dBm; % 計算 interference SNR
        % dprintf(sprintf('User %d Rx Power: %.2f dBm, SNR: %.2f dB\n', i, P_tx_dBm + friis_equation(freq, G_tx(i), 1, rx_Dists(i)), SNR_dB(i)));
    end
