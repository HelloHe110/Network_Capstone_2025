...
// Task 1-1:
template <typename T>
void NixVectorHelper<T>::SetPathFile (std::string pathFile) {
  m_pathFile = pathFile;
}
...
{
  // Task 1-1:
  auto helper = new NixVectorHelper<T> (*this);
  helper->SetPathFile (this->m_pathFile);
  return helper;
}
